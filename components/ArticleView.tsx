    ...
    <div className="fixed inset-0 z-[60] overflow-y-auto bg-background/98 backdrop-blur-xl animate-in fade-in duration-300">
      <div className="max-w-4xl mx-auto min-h-screen relative shadow-2xl bg-background border-x border-border flex flex-col">
        <div className="sticky top-0 z-20 glass border-b border-border flex justify-between items-center px-6 py-4">
          ...
          <button onClick={onClose} className="p-2 bg-card border border-border rounded-full hover:rotate-90 transition-transform">
        ...
        <div className="px-6 md:px-16 py-10 flex-1">
          <div className="flex items-center gap-2 mb-6">
            <span className="bg-primary/20 text-primary px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest">{article.category}</span>
            <span className="text-textSecondary text-xs">Publié le {new Date(article.publishedAt).toLocaleDateString('fr-FR', { dateStyle: 'long' })}</span>
          </div>

          <h1 className="text-3xl md:text-5xl font-display font-extrabold leading-tight mb-8">{article.title}</h1>

          <div className="mb-10 p-6 rounded-2xl bg-indigo-500/5 border border-primary/20 relative">
          ...
          <div className="flex flex-col gap-8">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 py-8 border-t border-border mt-8">
              <a href={article.url} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto text-center px-8 py-4 bg-primary hover:bg-indigo-600 rounded-full font-bold text-white transition-all shadow-lg shadow-primary/30">
                Lire l'article original sur {article.source}
              </a>
              <div className="flex items-center gap-4">
                 <button className="p-3 rounded-full bg-card border border-border hover:text-accent transition-colors"><ICONS.Heart /></button>
                 <button className="p-3 rounded-full bg-card border border-border hover:text-primary transition-colors"><ICONS.Bookmark active={article.isBookmarked} /></button>
                 <button className="p-3 rounded-full bg-card border border-border hover:text-white transition-colors"><ICONS.Share /></button>
              </div>
            </div>
          </div>
        </div>
